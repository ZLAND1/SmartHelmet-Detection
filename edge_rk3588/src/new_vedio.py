...
def draw(image, boxes, scores, classes, output_stracks):
    colors = [(0, 0, 255), (255, 0, 0), (255, 255, 255)]
    for box, score, cl ,id in zip(boxes, scores, classes, output_stracks):
        top, left, right, bottom = [int(_b) for _b in box]
        print("%s @ (%d %d %d %d) %.3f" % (CLASSES[cl], top, left, right, bottom, score))
        color = colors[cl]
        cv2.rectangle(image, (top, left), (right, bottom), color, 2)
        cv2.putText(image, '{0}-{1} {2:.2f}'.format(id,CLASSES[cl], score),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path', default='../model/yolo11_best.rknn', type=str,  help='model path, rknn file')
    parser.add_argument('--npu_core', default=0, type=int,  help='0,1,2')
    parser.add_argument('--input_stream', default='../image/test.mp4', type=str,  help='rtsp path or video index')
    parser.add_argument('--mot20', default=False, type=int,  help='')
    parser.add_argument('--track_thresh', default=0.4, type=int,  help='跟踪的阈值')
    parser.add_argument('--match_thresh', default=0.4, type=int,  help='匹配阈值')
    parser.add_argument('--track_buffer', default=60, type=int,  help='跟踪缓冲区大小')    
    ...
    try:
        ...
        while True:
            ...
            if boxes is not None:
                ...
                output_stracks = tracker.update(boxes, scores)
                draw(img_src, boxes, scores, classes, output_stracks)

            # 显示处理后的图像
            cv2.imshow('YOLOv11 Detection', img_src)
